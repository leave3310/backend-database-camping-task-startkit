...
DELETE FROM "USER"
WHERE "email" = 'opacity0@hexschooltest.io' AND "role" = 'USER';
...
SET 
    experience_years = 3
WHERE id = (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'));

UPDATE "COACH" 
SET 
    experience_years = 5
WHERE id = (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io'));
...
SET 
   ...
   status = '課程已取消'
WHERE id = (SELECT id FROM "COURSE_BOOKING" WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明'));
